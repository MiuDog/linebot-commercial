...
| 位數 | 何時遞增 | 本專案的實例 |
|---|---|---|
| **主版本** | 不相容變更：既有指令語法改變、資料表結構破壞性異動、`downloads/` 目錄結構改變 | 若把 `zd` 編號格式改掉，或改變實體資料夾層級 |
...
**資料不會回滾**——`downloads/` 與 `assets.db` 是營運資料，與程式版本無關。若該版本曾異動資料表結構，回滾前必須確認舊版程式能讀新版結構，否則要先還原資料備份。
