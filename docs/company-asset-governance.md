...
本地保全包可作為匯入來源：先核對 `preservation-manifest.json` 雜湊，從原始範本取得 Logo、印章及固定聯絡資料，以原始品項分析和 Excel 單價交叉核對後建立 `ITEM_MASTER`。啟用前先備份資料庫及現有主檔 CSV；資產及備份均留在本地忽略目錄，不加入 Git 或映像。

產檔會保留範本明訂列高，並將每張由程式分頁的工作表縮放至一個列印頁，避免 LibreOffice 使用固定比例而切開備註欄、或自動縮列後截斷印章。母檔本身不會因此改寫；驗證必須檢查實際 PDF，不能只確認 XLSX 存在。
