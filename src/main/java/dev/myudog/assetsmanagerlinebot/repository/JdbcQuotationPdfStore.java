...
import dev.myudog.assetsmanagerlinebot.service.quotation.QuotationPdfStore;
...
public class JdbcQuotationPdfStore implements QuotationPdfStore {
	...
	// 方法：建立報價 PDF 狀態儲存庫。
	public JdbcQuotationPdfStore(JdbcTemplate jdbc) {
		this.jdbc = jdbc;
	}
	...
	public Optional<PdfJob> find(long quotationId) {
		...
		List<Map<String, Object>> rows = jdbc.queryForList("""
			SELECT q.id, q.quotation_no, q.status, qf.relative_path
			FROM quotation q
	...
	public void markReady(long quotationId, String relativePdfPath, String contentHash, long fileSize) {
		int fileUpdated = jdbc.update("""
			...
			""", relativePdfPath, contentHash, fileSize, quotationId);
