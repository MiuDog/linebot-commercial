...
SET workbook_path = 'src/test/resources/quotation/templates/quotation-template-CNS.xlsx',
    detail_last_row = 31,
    subtotal_cell = 'G32',
    pre_tax_cell = 'G33',
    tax_cell = 'G34',
...
SET workbook_path = CASE template_key
        WHEN 'GENERAL_V1' THEN 'src/test/resources/quotation/templates/quotation-template-GENERAL.xlsx'
        WHEN 'MARINE_V1' THEN 'src/test/resources/quotation/templates/quotation-template-MARINE.xlsx'
    END,
    updated_at = CURRENT_TIMESTAMP
...
-- Synthetic regression prices: item order × 10, independent of company price lists.
WITH excel_master (scheme_code, item_code, specification, unit, unit_price, remark, display_order) AS (
    SELECT s.code, i.code, 'TEST', '式', i.id * 10, 'TEST ONLY', i.id
    FROM quotation_scheme s CROSS JOIN quotation_item i
    WHERE s.code IN ('CNS', 'GENERAL') AND (s.code = 'CNS' OR i.code <> 'ELEVATOR_GATE')
)
